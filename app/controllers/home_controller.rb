class HomeController < ApplicationController
  ...
  def prepare_chart_data
    ...
    @status_distribution_data = {
      ...
      series: [
        {
          name: 'Migrados',
          data: @kinds.map { |kind| @kinds_stats[kind][:migrated] }
        },
        {
          name: 'Pendentes',
          data: @kinds.map { |kind| @kinds_stats[kind][:not_migrated] }
        }
      ]
    }
  ...
  def get_daily_migration_data
    ...
    orders.each do |order|
      ...
      if date >= @month_ago && date <= @today
        ...
        if data_by_kind_and_date[order.kinds]
          data_by_kind_and_date[order.kinds][formatted_date] += 1
        end
      end
    end
    ...
    series = @kinds.map do |kind|
      {
        name: kind_display_name(kind),
        data: formatted_dates.map { |date| data_by_kind_and_date[kind][date[:full]] || 0 }
      }
    end
    
    {
      dates: formatted_dates.map { |d| d[:short] },
      full_dates: formatted_dates,
      series: series
    }
  end
  ...
  def calculate_completion_forecast
    ...
    
    {
      ...
      series: [
        {
          name: 'Dias estimados para conclusão',
          data: @kinds.map { |kind| completion_days[kind] }
        }
      ],
