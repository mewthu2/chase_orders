class CreateShopifyOrdersFromTinyJob < ActiveJob::Base
  ...
  def create_draft_order(session:, line_items:, cliente:, customer_phone:, endereco:, pedido:, source_name: nil)
    ...
    input = {
      ...
      'note' => "#{pedido['forma_pagamento']} - #{pedido['meio_pagamento']}" || '',
    ...
    begin
      ...
      order_data = {
        order: {
          ...
          customer: if draft_order['customer'].present? && draft_order['customer'].dup['id'] != @stock_client_id
                      customer_data = draft_order['customer'].dup
                      customer_data['first_name'] ||= @first_name
                      customer_data['last_name'] ||= @last_name
                      customer_data['id'] ||= @stock_client_id
                      customer_data['email_marketing_consent'] ||= {
                        state: 'subscribed',
                        opt_in_level: 'single_opt_in',
                        consent_updated_at: Time.now.iso8601
                      }
