  ...
  def self.create_orders(params, attempt)
    ...
    if request.present?
      ...
      if request['statusCode'] == 200
        ...
        attempt.update(status: :success, order_correios_id: /ID: (\d+)/.match(request['mensagem'])[1]) if request['mensagem'].include?('Pedido já Cadastrado') && tracking['rastreio'][0].present?
      end
    else
